...
// TestFloat32UnmarshalJSON ...
func TestFloat32UnmarshalJSON(t *testing.T) {

	type testStruct struct {
		Req float32 `json:"req"`
		Opt Float32 `json:"opt"`
	}

	toPtr := func(f float32) *float32 {
		return &f
	}
	...
	success := map[string]struct {
		...
		"has key": {
			...
			want: testStruct{
				Req: 100.12,
				Opt: Float32{v: toPtr(200.23), hasKey: true},
			},
		},
		"has key(is initial value)": {
			...
			want: testStruct{
				Req: 0,
				Opt: Float32{v: toPtr(0), hasKey: true},
			},
		},
		"has key(is null)": {
