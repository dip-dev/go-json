...
func TestUint8UnmarshalJSON(t *testing.T) {
	...
	success := map[string]struct {
		...
		"has key": {
			...
			want: testStruct{
				Req: 10,
				Opt: Uint8{v: toPtr(20), hasKey: true},
			},
		},
		"has key(is initial value)": {
			...
			want: testStruct{
				Req: 0,
				Opt: Uint8{v: toPtr(0), hasKey: true},
			},
		},
		"has key(is null)": {
...
// TestUint8MarshalJSON ...
func TestUint8MarshalJSON(t *testing.T) {
	type testStruct struct {
		Req uint8  `json:"req"`
		Opt *Uint8 `json:"opt,omitempty"`
	}
	...
	success := map[string]struct {
		...
		"has key": {
			data: testStruct{
				Req: 100,
				Opt: &Uint8{v: toPtr(200), hasKey: true},
			},
